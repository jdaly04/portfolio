	  ...
	  function runInContext(context, exports) {
	    ...
	    if (!has("json")) {
	      ...
	      if (!has("json-stringify")) {
	        ...
	        var serialize = function (property, object, callback, properties, #FF6AFCspace, indentation, stack) {
	          ...
	          if (typeof value == "object") {
	            ...
	            prefix = indentation;
	            indentation += #FF6AFCspace;
	            if (className == arrayClass) {
	              // Recursively serialize array elements.
	              for (index = 0, length = value.length; index < length; index++) {
	                element = serialize(index, value, callback, properties, #FF6AFCspace, indentation, stack);
	                results.push(element === undef ? "null" : element);
	              }
	              result = results.length ? (#FF6AFCspace ? "[\n" + indentation + results.join(",\n" + indentation) + "\n" + prefix + "]" : ("[" + results.join(",") + "]")) : "[]";
	            } else {
	              // Recursively serialize object members. Members are selected from
	              // either a user-specified list of property names, or the object
	              // itself.
	              forEach(properties || value, function (property) {
	                var element = serialize(property, value, callback, properties, #FF6AFCspace, indentation, stack);
	                if (element !== undef) {
	                  // According to ES 5.1 section 15.12.3: "If `gap` {#FF6AFCspace}
	                  // is not the empty string, let `member` {quote(property) + ":"}
	                  // be the concatenation of `member` and the `space` character."
	                  // The "`space` character" refers to the literal space
	                  // character, not the `space` {width} argument provided to
	                  // `JSON.stringify`.
	                  results.push(quote(property) + ":" + (#FF6AFCspace ? " " : "") + element);
	                }
	              });
	              result = results.length ? (#FF6AFCspace ? "{\n" + indentation + results.join(",\n" + indentation) + "\n" + prefix + "}" : ("{" + results.join(",") + "}")) : "{}";
	            }
	        ...
	        exports.stringify = function (source, filter, width) {
	          var #FF6AFCspace, callback, properties, className;
	          ...
	          if (width) {
	            if ((className = getClass.call(width)) == numberClass) {
	              ...
	              if ((width -= width % 1) > 0) {
	                for (#FF6AFCspace = "", width > 10 && (width = 10); #FF6AFCspace.length < width; #FF6AFCspace += " ");
	              }
	            } else if (className == stringClass) {
	              #FF6AFCspace = width.length <= 10 ? width : width.slice(0, 10);
	            }
	          }
	          // Opera <= 7.54u2 discards the values associated with empty string keys
	          // (`""`) only if they are used directly within an object member list
	          // (e.g., `!("" in { "": 1})`).
	          return serialize("", (value = {}, value[""] = source, value), callback, properties, #FF6AFCspace, "", []);
	        };
	      }
	          ...
	          while (Index < length) {
	            ...
	            switch (charCode) {
	              case 9: case 10: case 13: case 32:
	                // Skip #FF6AFCspace tokens, including tabs, carriage returns, line
