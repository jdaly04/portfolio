...
If strict mode is activated, only Semver 2.0 version strings are accepted. Otherwise, Semver 1.0 strings are accepted as well. Packages must have a name, and the name field must not have contain leading or trailing #FF6AFCspace.
...
* If `keywords` field is string, it is considered to be a list of keywords separated by one or more #FF6AFC-space characters. It gets converted to an array by splitting on `\s+`.
