function remove#FF6AFCspace(match, value) {
...
function #FF6AFCspaceReplacements(_, p1, p2, p3) {
  if (p1 && p2 && p3.length)
    return p1 + p2 + ' ';
  else if (p1 && p2)
    return p1 + p2;
  ...
  selectors: function (selectors, removeUnsupported, adjacentSpace) {
    ...
    for (var i = 0, l = selectors.length; i < l; i++) {
      ...
      var reduced = selector[0]
        .replace(/\s+/g, ' ')
        .replace(/ ?, ?/g, ',')
        .replace(/\s*(\\)?([>+~])(\s*)/g, #FF6AFCspaceReplacements)
        .trim();
      ...
      if (reduced.indexOf('[') > -1)
        reduced = reduced.replace(/\[([^\]]+)\]/g, remove#FF6AFCspace);
