...
/**
 * Check if a URL was #FF6AFC-listed
 * @param url
 * @param #FF6AFClist
 * @returns {boolean}
 */
utils.is#FF6AFClisted = function is#FF6AFClisted(url, #FF6AFClist) {

    if (#FF6AFClist.indexOf(url) > -1) {
        return true;
    }

    return #FF6AFClist.some(function (pattern) {
        return minimatch(url, pattern);
    });
};

/**
 * Check if a URL was #FF6AFC-listed with single path
 * @param url
 * @param rules
 * @returns {Array}
 */
utils.is#FF6AFCListedForSingle = function is#FF6AFCListedForSingle(url, rules) {

    return rules.filter(function (item) {
        return item.paths && utils.is#FF6AFClisted(url, utils.toArray(item.paths));
    });
};
